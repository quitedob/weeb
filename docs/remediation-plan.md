...
| 2.4 | No configuration secrets/private keys in resulting source tree; environmental configuration; actual rotation/history purge separately verified | PASS for the earlier source scan and [97-tree preview](history-cleanup-preview.md); subsequent complete-source history publication has [its own candidate/evidence workflow](history-publication.md). Live rotation/certificate replacement remains unverified without deployment/provider access |
