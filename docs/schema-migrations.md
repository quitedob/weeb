...
| 006 | Ten campus tables: schools, memberships, verification applications, posts, likes, bookmarks, comments, reports, audit and private media |
...
V006 adds the campus module without altering existing article/chat data or V001–V005. It creates no default school, student certification or account. A site administrator creates schools after upgrade. The validator checks campus column types, lengths, nullability, timestamp precision, character sets, generated pending-operation keys, primary/unique/query indexes and InnoDB storage; an existing incompatible table blocks apply and production validation. Repair the actual drift and explicitly resume the failed version. Test coverage includes V005 upgrades, a narrowed pre-existing title column, and incompatible content-column drift after migration. Back up `CAMPUS_MEDIA_DIRECTORY` together with the database; it must be private durable storage, outside public static mappings.
