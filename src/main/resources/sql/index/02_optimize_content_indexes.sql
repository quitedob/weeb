...
-- 优化文章作者查询
CREATE INDEX idx_article_author_status
ON articles (user_id, status, created_at DESC);

-- 优化文章审核优先级查询
CREATE INDEX idx_article_moderation
...
-- 优化文章评论查询
CREATE INDEX idx_article_comment_time
ON article_comment (article_id, created_at DESC);

-- 8. user_follow表索引优化
-- 优化关注关系查询
CREATE INDEX idx_user_follow_relation
ON user_follow (follower_id, followee_id);

-- 优化粉丝查询
CREATE INDEX idx_user_follow_followee
