...
-- 1. message表索引优化
-- 优化私聊消息查询
CREATE INDEX idx_message_private_chat_optimized
ON message (sender_id, receiver_id, created_at DESC);

-- 优化群聊消息查询
CREATE INDEX idx_message_group_chat_optimized
...
-- 2. chat_list表索引优化
-- 优化用户聊天列表查询
CREATE INDEX idx_chat_list_user_update
ON chat_list (user_id, update_time DESC);

-- 优化聊天类型查询
CREATE INDEX idx_chat_list_user_type
ON chat_list (user_id, type, update_time DESC);

-- 3. group_member表索引优化
-- 优化用户所在群组查询
CREATE INDEX idx_group_member_user_status
...
-- 优化好友状态查询
CREATE INDEX idx_contact_user_status
