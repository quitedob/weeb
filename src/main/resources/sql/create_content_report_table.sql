...
-- 添加外键约束（如果需要）
-- ALTER TABLE `content_report` ADD CONSTRAINT `fk_report_reporter`
--     FOREIGN KEY (`reporter_id`) REFERENCES `user` (`id`) ON DELETE CASCADE;
-- ALTER TABLE `content_report` ADD CONSTRAINT `fk_report_reviewer`
--     FOREIGN KEY (`reviewer_id`) REFERENCES `user` (`id`) ON DELETE SET NULL;

-- 插入示例数据（可选）
-- INSERT INTO `content_report` (reporter_id, content_type, content_id, reason, description) VALUES
-- (1, 'article', 1, 'spam', 'This is spam content'),
-- (2, 'comment', 5, 'inappropriate_content', 'Inappropriate language used');