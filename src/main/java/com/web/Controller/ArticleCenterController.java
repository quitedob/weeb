...
public class ArticleCenterController {
    ...
    /**
     * 增加粉丝（订阅用户）
     * 请求示例：POST /articles/subscribe?userId=123&targetUserId=456
     */
    ...
    public ResponseEntity<String> subscribe(
        ...
        try {
            boolean success = articleService.subscribeUser(userId, targetUserId);
            if (!success) {
                return ResponseEntity.badRequest().body("订阅失败");
            }
            return ResponseEntity.ok("订阅成功");
        } catch (Exception e) {
            logger.error("订阅失败在用户 {} 和 {}之间", userId, targetUserId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("互联网服务失败");
        }
    }
    ...
    public ResponseEntity<String> editArticle(
        ...
        try {
            boolean updated = articleService.editArticle(id, article);
            if (!updated) {
    ...
    public ResponseEntity<String> deleteArticle(@RequestParam("id") Long id) {
        try {
            boolean deleted = articleService.deleteArticle(id);
            if (deleted) {
                return ResponseEntity.ok("文章删除成功");
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("未找到该文章，删除失败");
            }
        } catch (Exception e) {
