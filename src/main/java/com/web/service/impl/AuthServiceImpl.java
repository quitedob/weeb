...
public class AuthServiceImpl implements AuthService {
    ...
    private final com.web.service.PasswordResetService passwordResetService;

    public AuthServiceImpl(AuthMapper authMapper, UserMapper userMapper, UserCreationService userCreationService, PasswordEncoder passwordEncoder, RedisTemplate<String, Object> redisTemplate, JwtUtil jwtUtil, com.web.service.PasswordResetService passwordResetService) {
    ...
    @Override
    public boolean changePassword(Long userId, String encodedPassword, String verifiedPreviousPassword) {
        if (userId == null || encodedPassword == null || verifiedPreviousPassword == null) {
            throw new WeebException("Password change requires verified credentials");
        }
        if (userMapper.compareAndSetPassword(userId, encodedPassword, verifiedPreviousPassword) != 1) return false;
    ...
    public void register(User user) {
        ...
        // 设置默认用户状态为启用
        user.setStatus(1);
        user.setType(com.web.constant.UserType.USER);
    ...
    public String login(String username, String password) {
        ...
        authMapper.updateLoginTime(user.getId(), user.getLoginTime());

        // 设置用户在线状态
        online(user.getId());

        userMapper.recordSuccessfulLoginDay(user.getId());
    ...
    public boolean sendPasswordResetEmail(String email) {
        return Boolean.TRUE.equals(passwordResetService.sendPasswordResetLink(email).get("success"));
    }

    @Override
    public boolean resetPassword(String token, String newPassword) {
        return Boolean.TRUE.equals(passwordResetService.resetPassword(token, newPassword, newPassword).get("success"));
    }

    @Override
    public boolean verifyResetToken(String token) {
        return Boolean.TRUE.equals(passwordResetService.validateResetToken(token).get("valid"));
    }
}
