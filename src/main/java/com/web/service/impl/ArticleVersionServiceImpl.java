...
public class ArticleVersionServiceImpl implements ArticleVersionService {
    ...
    public boolean createVersion(Long articleId, String title, String content, Long userId) {
        userId = requireEditor(articleId, userId);
        try {
            // 获取下一个版本号
    ...
    private Long requireEditor(Long articleId, Long suppliedUserId) {
        String username = SecurityUtils.getCurrentUsername();
        User actor = username == null ? null : authMapper.findByUsername(username);
        if (actor == null || !Integer.valueOf(1).equals(actor.getStatus())
                || (suppliedUserId != null && !actor.getId().equals(suppliedUserId))) {
            throw new AccessDeniedException("Authenticated editor required");
        }
        Article article = articleId == null ? null : articleMapper.selectArticleById(articleId);
        if (article == null || (!actor.getId().equals(article.getUserId())
                && !UserType.ADMIN.equals(UserType.normalize(actor.getType())))) {
            throw new AccessDeniedException("Article versions are restricted to the author and administrators");
        }
        return actor.getId();
    }
