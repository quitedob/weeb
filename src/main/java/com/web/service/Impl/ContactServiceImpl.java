...
public class ContactServiceImpl implements ContactService {
    ...
    public void apply(ContactApplyVo applyVo, Long fromUserId) {
        ...
        if (existingContact != null) {
            if (ContactStatus.ACCEPTED.getCode().equals(existingContact.getStatus())) {
                throw new WeebException("You are already friends.");
            }
            if (ContactStatus.PENDING.getCode().equals(existingContact.getStatus())) {
            ...
            if (ContactStatus.BLOCKED.getCode().equals(existingContact.getStatus())) {
    ...
    public void accept(Long contactId, Long toUserId) {
        ...
        if (!ContactStatus.PENDING.getCode().equals(contactRequest.getStatus())) {
    ...
    public void declineOrBlock(Long contactId, Long currentUserId, ContactStatus newStatus) {
        if (newStatus != ContactStatus.DECLINED && newStatus != ContactStatus.BLOCKED) {
            throw new WeebException("Invalid status for this operation. Only DECLINED or BLOCKED allowed.");
        }
        ...
        boolean isReceiverOfPending = ContactStatus.PENDING.getCode().equals(contact.getStatus()) && contact.getFriendId().equals(currentUserId);
        boolean isPartOfAccepted = ContactStatus.ACCEPTED.getCode().equals(contact.getStatus()) && (contact.getUserId().equals(currentUserId) || contact.getFriendId().equals(currentUserId));
