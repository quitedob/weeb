...
public class ArticleCenterController {
    ...
    public ResponseEntity<ApiResponse<String>> addCoin(
            ...
            @Userid Long userId) {
        try {
            boolean updated = articleService.addCoin(id, amount, userId);
            if (!updated) {
                return ResponseEntity.badRequest()
        ...
        } catch (com.web.exception.WeebException e) {
            return ResponseEntity.badRequest().body(ApiResponse.coinOperationFailed(e.getMessage()));
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<String>> favoriteArticle(@PathVariable Long id, @Userid Long authenticatedUserId) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ApiResponse.error("无权收藏此文章"));
        } catch (Exception e) {
