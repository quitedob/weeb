...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<String>> changePassword(
        ...
        try {
            ...
            String token = authorization.substring(7);
            if (!jwtUtil.validateToken(token)) return ApiResponseUtil.unauthorizedString("令牌无效或已过期");
            ...
            if (!ValidationUtils.validatePassword(passwordChangeVo.getNewPassword())) {
            ...
            // 更新密码
            String verifiedPreviousPassword = user.getPassword();
            user.setPassword(passwordEncoder.encode(passwordChangeVo.getNewPassword()));
            boolean updated = authService.changePassword(user.getId(), user.getPassword(), verifiedPreviousPassword);

            if (updated) {
                // 将所有令牌加入黑名单，强制重新登录
                // Sessions were revoked by the password update service.
    ...
    public ResponseEntity<ApiResponse<String>> forgotPassword(@RequestBody @Valid ForgotPasswordVo forgotPasswordVo) {
        try {
            ...
            if (sent) {
                return ApiResponseUtil.successString("如果该邮箱已注册，重置链接已发送");
            } else {
