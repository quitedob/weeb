...
public class UserController {
    ...
    @Autowired
    private com.web.service.UserFollowService userFollowService;

    @Autowired
    private com.web.service.UserPreferencesService preferencesService;

    @Autowired
    private com.web.service.AvatarStorageService avatarStorageService;

    @Autowired
    private com.web.service.UserLevelService userLevelService;

    @GetMapping("/me/level/upgrade-progress")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getUpgradeProgress(@Userid Long userId) {
        if (userId == null) return ApiResponseUtil.badRequestMap("用户未认证");
        return ApiResponseUtil.successMap(userLevelService.getUpgradeProgress(userId));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> uploadAvatar(@RequestParam("file") MultipartFile file) {
        ...
        try {
            ...
            if (userId == null) return ApiResponseUtil.badRequestMap("用户未认证");
            url = avatarStorageService.store(file);
            User user = new User();
            user.setId(userId);
            user.setAvatar(url);
            if (!userService.updateUserBasicInfo(user)) throw new IllegalStateException("头像保存失败");
            return ApiResponseUtil.successMap(Map.of("avatar", url));
        } catch (Exception e) {
            if (url != null) {
                try { avatarStorageService.remove(url); } catch (java.io.IOException cleanupFailure) {
                    log.warn("Failed to remove an unused avatar file");
                }
            }
            return ApiResponseUtil.handleServiceExceptionMap(e, "上传头像");
        }
    }
    ...
    public ResponseEntity<ApiResponse<String>> resetUserPassword(
        ...
        try {
            ...
            String sanitizedPassword = newPassword;
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getUserFollowing(
            @PathVariable Long userId,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "20") int pageSize) {
        try {
            if (!userId.equals(SecurityUtils.getCurrentUserId()) && !preferencesService.isFollowingVisible(userId)) {
                return ResponseEntity.status(403).body(ApiResponse.error(ApiResponse.ErrorCode.FORBIDDEN, "该用户已隐藏关注列表"));
            }
            return ApiResponseUtil.successMap(userFollowService.getFollowingList(userId, page, pageSize));
        } catch (Exception e) {
            return ApiResponseUtil.handleServiceExceptionMap(e, "获取用户关注列表", userId, page, pageSize);
        }
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getUserFollowers(
            @PathVariable Long userId,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "20") int pageSize) {
        try {
            // 这里需要实现获取粉丝列表的逻辑
            return ApiResponseUtil.successMap(userFollowService.getFollowersList(userId, page, pageSize));
        } catch (Exception e) {
            return ApiResponseUtil.handleServiceExceptionMap(e, "获取用户粉丝列表", userId, page, pageSize);
        }
    }
    ...
    public ResponseEntity<ApiResponse<Boolean>> checkFollowStatus(
        ...
        try {
            ...
            // 这里需要实现检查关注状态的逻辑
            return ApiResponseUtil.successBoolean(userFollowService.isFollowing(currentUserId, userId));
        } catch (Exception e) {
