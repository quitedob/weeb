...
const goToMessage = (message) => {
  ...
  if (!sharedChatId) {
    ElMessage.error('无法打开该消息的会话');
    return;
  }
  const type = message.type === 'GROUP' ? 'group' : 'private';
  const groupId = message.groupId ?? message.targetId;
  router.push({
    name: 'SpecificChat',
    params: { type, id: String(sharedChatId) },
    query: type === 'group' && groupId ? { groupId: String(groupId) } : {},
  });
};
