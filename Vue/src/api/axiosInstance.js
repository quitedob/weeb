...
export const instance = axios.create({
    baseURL: '/api', // All requests are based on this path
...
instance.interceptors.response.use(
    response => {
        ...
        // Backend ApiResponse structure usually includes code, message, data
        if (res.code && res.code !== 200) {
            ElMessage({
                message: res.message || 'Error From API', // Added "From API" for clarity
                type: 'error',
                duration: 5 * 1000
            });

            // Example: specific error code handling, e.g., token expired
            if (res.code === 401 || res.code === 403) { // Assuming 401 for token expired or unauthorized
                try {
                    const authStore = useAuthStore();
                    authStore.logout(); // Clear frontend state
                } catch (e) {
                    console.warn('useAuthStore not available in response interceptor for logout:', e);
                    localStorage.removeItem('jwt_token'); // Manual cleanup if store fails
                }
                router.push('/login'); // Redirect to login page
            }
            return Promise.reject(new Error(res.message || 'Error From API'));
        }
        return res; // Return res (backend ApiResponse object)
    },
    error => {
