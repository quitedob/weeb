...
async function getUserMap() {
  try {
    const res = await axios.get('/api/v1/user/map')
    if (res.data.code === 0) {
...
async function getOnlineUsers() {
  try {
    const res = await axios.get('/api/v1/user/online')
    if (res.data.code === 0) {
...
async function fetchChatRecord() {
  try {
    ...
    const res = await axios.post('/api/v1/message/record', payload)
    if (res.data.code === 0) {
...
async function handlerSubmitMsg() {
  ...
  try {
    ...
    const response = await axios.post('/api/v1/message/send', payload)
...
async function recallMessage(msgId) {
  try {
    ...
    const res = await axios.post('/api/v1/message/recall', payload)
